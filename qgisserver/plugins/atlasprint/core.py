...
def print_layout(
    ...
    **kwargs,
    ...
    if master_layout.layoutType() == QgsMasterLayoutInterface.PrintLayout:
        ...
        if (
            not scales
            and atlas_layout.referenceMap().atlasScalingMode()
    ...
    for key, value in kwargs.items():
        ...
        logger.info(
            'Additional parameters "{key}" {found} in layout, value "{value}"'.format(
                key=key, found="found" if found else "not found", value=value
            )
        )

    file_name = "{}_{}.{}".format(
        clean_string(layout_name), uuid4(), output_format.name.lower()
    )
    export_path = Path(tempfile.gettempdir()).joinpath(file_name)

    Logger().info(
        "Exporting the request in {} using {}".format(export_path, output_format.value)
    )
    ...
    if output_format in (OutputFormat.Png, OutputFormat.Jpeg):
        ...
        result, error = QgsLayoutExporter.exportToPdf(
            atlas or report_layout, str(export_path), settings
        )
    ...
    if result != QgsLayoutExporter.Success:
        raise Exception(
            "Export not generated in QGIS exporter {} : {}".format(export_path, error)
        )
    ...
    if not export_path.is_file():
        ...
        raise Exception(
            "Export OK from QGIS, but file not found on the file system : {}".format(
                export_path
            )
        )
...
def parse_output_format(output: Union[str, None]) -> OutputFormat:
    ...
    # Default value
    Logger().info(
        'Output format is invalid, default to PDF. It was "{}"'.format(output)
    )
    return OutputFormat.Pdf
...
def optimize_expression(layer, expression):
    ...
    if expression.find("$id") < 0:
        logger.info(
            "'$id' not found in the expression, returning the input expression."
        )
        return expression
    ...
    if len(primary_keys) != 1:
        logger.info(
            "Primary keys are not defined in the layer '{}'.".format(layer.id())
        )
        return expression
    ...
    if not field.isNumeric():
        logger.info(
            "The field '{}' is not numeric in layer '{}'.".format(
                field.name(), layer.id()
            )
        )
        return expression
    ...
    logger.info(
        '$id has been replaced by "{}" in layer "{}"'.format(field.name(), layer.id())
    )
